# -*- coding: utf-8 -*-
"""
Created on Wed Aug 21 15:46:14 2024
...
class PEC_Cell():
    ...
    def Set_scenario(self,scenario):
        ...
        if self.PEC_mode=='Solar cells':
            ...
            if self.Three_solar_cells:
                ...
                self.grid_step=np.array([0.025,0.025])
                if self.set_voltage_max_ethylene >-0.5:
                    self.bandgap_range=np.array([1.5,2.0,0.9,1.3])
                elif self.set_voltage_max_ethylene >-1:
                    self.bandgap_range=np.array([1.6,2.2,1.2,1.7])
                else:
                    self.bandgap_range=np.array([2.0,2.5,1.5,2.1])
            else:
                self.bandgap_range=np.array([1.6,2.4,1.0,1.9])
                self.grid_step=np.array([0.05,0.05])   
    ...
    def Data_CO2RR_catalyst(self):
        ...
        elif self.CO2RR_catalyst=='Variable':
            Voltage=[self.set_voltage_max_ethylene-0.01,self.set_voltage_max_ethylene]
            Ethylene_percentage=[self.set_max_ethylene,self.set_max_ethylene]
            Current_density=[249,250]
        else:
...
unit=' $\mu$mol/h/cm$^2$ C$_2$H$_4$'
cmap='Purples_r'
levels=np.arange(0,80,5)
clabel='C$_2$H$_4$ production rate ($\mu$mol/h/cm$^2$)'
figcontour, ax = plt.subplots()
  
    
CS1 = ax.contourf(X, Y, umol_production,levels=levels,cmap=cmap)
...
CS1 = ax.contourf(X, Y, STE_production,levels=levels,cmap=cmap)
